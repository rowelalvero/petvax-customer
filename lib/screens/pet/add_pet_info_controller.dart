...
class AddPetInfoController extends GetxController {
  ...
  Rx<Future<List<ChoosePetModel>>> choosePets =
      Future(() => <ChoosePetModel>[]).obs;
  ...
  RxBool isBreedUpdate = false.obs;
  ...
  List<AddPetPageElementModel> pageViewElementList = [
    AddPetPageElementModel(
        midImg1: Assets.dogPawDogPaw,
        appBarTitle: locale.value.selectYourPet,
        midImg2: Assets.dogPawDogPaw1,
        isStep2Active: false,
        isStep3Active: false),
    AddPetPageElementModel(
        midImg1: Assets.dogPawDogPaw2,
        appBarTitle: locale.value.addPetInfo,
        midImg2: Assets.dogPawDogPaw,
        isStep2Active: true,
        isStep3Active: false),
    AddPetPageElementModel(
        midImg1: Assets.dogPawDogPaw2,
        appBarTitle: locale.value.additionalInfo,
        midImg2: Assets.dogPawDogPaw2,
        isStep2Active: true,
        isStep3Active: true),
  ];
  ...
  init() {
    try {
      ...
      if (Get.arguments is PetData) {
        ...
        selectedHeightUnit(heightUnits.firstWhere(
            (element) => element.value
                .toLowerCase()
                .contains(petProfileData.value.heightUnit.toLowerCase()),
            orElse: () => defaulHEIGHT.value));
        selectedWeightUnit(weightUnits.firstWhere(
            (element) => element.value
                .toLowerCase()
                .contains(petProfileData.value.weightUnit.toLowerCase()),
            orElse: () => defaulWEIGHT.value));
        ...
        birthdayCont.text = petProfileData
            .value.dateOfBirth.dateInyyyyMMddFormat
            .formatDateYYYYmmdd();
  ...
  void showConfimDialogChoosePhoto() {
    showConfirmDialogCustom(
      ...
      title:
          "${locale.value.wouldYouLikeToSetPictureAs} ${myPetsScreenController.selectedPetProfile.value.name}${locale.value.sProfilePicture}",
    );
  }
  ...
  addPetApi() async {
    isLoading(true);
    hideKeyBoardWithoutContext();
    addPetReq.breedId = isBreedUpdate.value
        ? selectedBreed.value.id
        : isUpdateProfile.value
            ? petProfileData.value.breedId
            : selectedBreed.value.id;
    ...
    if (imageFile.value.path.isNotEmpty) {
      addPetReq.petImages = [XFile(imageFile.value.path)];
    }
    ...
    PetService.addPetDetailsApi(
      ...
      onPetAdd: () {
        ...
        if (isFromBookings.value || isUpdateProfile.value || isProfilePhoto) {
          ...
          toast(
              "${locale.value.petProfileUpdate} ${locale.value.successfully}");
          Get.back();
          if (isUpdateProfile.value) {
            Get.back();
          }
          ...
          myPetsScreenController.init();
          Get.offUntil(
              GetPageRoute(page: () => AddPetSuccessScreen()),
              (route) =>
                  route.isFirst || route.settings.name == '/MyPetsScreen');
        }
      },
      loaderOff: () {
        isLoading(false);
  ...
  getPetTypesApi() async {
    ...
    PetService.getPetTypeApi(
      ...
      if (isUpdateProfile.value) {
        selectedPetType(petTypeList.firstWhere(
            (p0) => p0.name
                .toLowerCase()
                .contains(petProfileData.value.pettype.toLowerCase()),
            orElse: () => ChoosePetModel()));
      }
      isLoading(false);
    }).catchError((e) {
      isLoading(false);
  ...
  getBreed({String searchtext = ""}) {
    ...
    isLoading(true);
    PetServiceFormApis.getBreed(
            petTypeId: selectedPetType.value.id, search: searchtext)
        .then((value) {
      isLoading(false);
      allBreedList(value.data);
      if (isUpdateProfile.value) {
        selectedBreed(allBreedList.firstWhere(
            (element) => petProfileData.value.breedId.toInt() == element.id,
            orElse: () => BreedModel()));
        breedList(allBreedList
            .where((p0) =>
                petProfileData.value.pettype.toLowerCase() ==
                p0.pettype.toLowerCase())
            .toList());
        breedCont.text = selectedBreed.value.name;
      } else {
        breedList(allBreedList
            .where((p0) =>
                selectedPetType.value.name.toLowerCase() ==
                p0.pettype.toLowerCase())
            .toList());
      }
