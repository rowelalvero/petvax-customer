...
class PetProfileScreen extends StatelessWidget {
    ...
    return AppScaffold(
            ...
            GestureDetector(
              onTap: () {
                Get.to(() => AddPetInfoScreen(),
                    arguments: myPetsScreenController.selectedPetProfile.value);
              },
      ...
      body: AnimatedScrollView(
        ...
        children: [
          ProfilePicWidget(
            heroTag:
                "${myPetsScreenController.selectedPetProfile.value.name}${myPetsScreenController.selectedPetProfile.value.id}",
            profileImage:
                myPetsScreenController.selectedPetProfile.value.petImage,
            ...
            subInfo:
                "${locale.value.breed}: ${myPetsScreenController.selectedPetProfile.value.breed}",
            onCameraTap: () {
              AddPetInfoController addPetInfoController = AddPetInfoController(
                  isProfilePhoto: true,
                  ...
                  child: Row(
                    ...
                    children: [
                      attributesWid(
                          ...
                          data: myPetsScreenController
                              .selectedPetProfile.value.gender
                              .capitalizeFirstLetter()),
                      const VerticalDivider(
                          color: borderColor, indent: 15, endIndent: 15),
                      attributesWid(
                          title: locale.value.birthday,
                          data: myPetsScreenController.selectedPetProfile.value
                              .dateOfBirth.dateInMMMMDyyyyFormat),
                      const VerticalDivider(
                          color: borderColor, indent: 15, endIndent: 15),
                      attributesWid(
                          title: locale.value.weight,
                          data:
                              "${myPetsScreenController.selectedPetProfile.value.weight.toString()} ${myPetsScreenController.selectedPetProfile.value.weightUnit.toString()}"),
                    ],
                  ),
                ),
              ).paddingSymmetric(horizontal: 16),
              if (!homeScreenController
                  .dashboardData.value.upcommingBooking.id.isNegative) ...[
                32.height,
                Text(locale.value.reminder, style: primaryTextStyle())
                    .paddingSymmetric(horizontal: 16),
                8.height,
                GestureDetector(
                  onTap: () {
                    Get.to(() => BookingDetailScreen(),
                        arguments: BookingDataModel(
                            id: homeScreenController
                                .dashboardData.value.upcommingBooking.id,
                            service: SystemService(
                                name: homeScreenController.dashboardData.value
                                    .upcommingBooking.service.name),
                          ...
                          children: [
                              ...
                              children: [
                                Text(
                                    getServiceNameByServiceElement(
                                        serviceSlug: homeScreenController
                                            .dashboardData
                                            .value
                                            .upcommingBooking
                                            .service
                            ...
                            Row(
                              ...
                              children: [
                                commonLeadingWid(
                                        imgPath:
                                            Assets.navigationIcCalendarOutlined,
                            ...
                            if (homeScreenController.dashboardData.value
                                .upcommingBooking.employeeName
                                .trim()
                                .isNotEmpty) ...[
                              GestureDetector(
                                onTap: homeScreenController.dashboardData.value
                                            .upcommingBooking.employeeId >
                                        0
                                    ? () {
                                ...
                                child: Row(
                                  children: [
                                    CachedImageWidget(
                                      url: homeScreenController.dashboardData
                                          .value.upcommingBooking.employeeImage,
                                      height: 20,
                                      width: 20,
                                      circle: true,
                                      radius: 20,
              ...
              Row(
                ...
                children: [
                  Row(
                    children: [
                      16.width,
                      Text('${locale.value.notesFor}  ',
                          style: primaryTextStyle()),
                      SizedBox(
                        width: 150,
                        child: Marquee(
                            child: Text(myPetsScreenController
                                .selectedPetProfile.value.name)),
                      ).center(),
                    ],
                  ),
                  TextButton(
                    onPressed: () {
                      petProfileController.titleCont.text = "";
                      petProfileController.noteCont.text = "";
                      petProfileController.selectedNote(NotePetModel(
                          createdbyUser: UserData(), isPrivate: false.obs));
                      Get.to(() => AddNote());
                    },
                    child: Text(locale.value.addNote,
                        style: primaryTextStyle(
                            color: primaryColor,
                  ...
                  onSuccess: (notes) {
                    return notes.isEmpty
                        ? NoDataWidget(
                            ...
                            subTitle:
                                "${locale.value.thereAreCurrentlyNoNotes} ${myPetsScreenController.selectedPetProfile.value.name}",
  ...
  String get scheduleDate {
    try {
      return homeScreenController.dashboardData.value.upcommingBooking
              .startDateTime.isValidDateTime
          ? homeScreenController.dashboardData.value.upcommingBooking
              .startDateTime.dateInDMMMMyyyyFormat
          : "";
    } catch (e) {
  ...
  String get scheduleTime {
    try {
      if (homeScreenController.dashboardData.value.upcommingBooking.service.slug
          .contains(ServicesKeyConst.boarding)) {
        return homeScreenController.dashboardData.value.upcommingBooking
                .dropoffDateTime.isValidDateTime
            ? " ${homeScreenController.dashboardData.value.upcommingBooking.dropoffDateTime.timeInHHmmAmPmFormat}"
            : "";
      } else {
        return homeScreenController.dashboardData.value.upcommingBooking
                .serviceDateTime.isValidDateTime
            ? " ${homeScreenController.dashboardData.value.upcommingBooking.serviceDateTime.timeInHHmmAmPmFormat}"
            : "";
      }
    } catch (e) {
