...
class DayCareServiceController extends GetxController {
  ...
  //DayCareTaker
  Rx<EmployeeModel> selectedDayCareTaker =
  ...
  void onInit() {
    ...
    if (Get.arguments is BookingDataModel) {
      try {
        ...
        addressCont.text = parseHtmlString(bookingFormData.value.address);
        ...
        favActCont.text = bookingFormData.value.activity.isNotEmpty
            ? bookingFormData.value.activity.first
            : "";
        favFoodCont.text = bookingFormData.value.activity.isNotEmpty
            ? bookingFormData.value.food.first
            : "";
        getDayCareTakers();
    ...
    try {
      ...
      dateCont.text = bDetailCont
          .bookingDetail.value.dayCareDate.dateInyyyyMMddFormat
          .formatDateDDMMYY();
      dropOffTimeCont.text =
          "${bDetailCont.bookingDetail.value.dayCareDate.dateInyyyyMMddFormat.formatDateYYYYmmdd()} ${bDetailCont.bookingDetail.value.dropoffTime}"
              .dateInyyyyMMddHHmmFormat
              .formatTimeHHmmAMPM();
      pickUpTimeCont.text =
          "${bDetailCont.bookingDetail.value.dayCareDate.dateInyyyyMMddFormat.formatDateYYYYmmdd()} ${bDetailCont.bookingDetail.value.pickupTime}"
              .dateInyyyyMMddHHmmFormat
              .formatTimeHHmmAMPM();

      bookDayCareReq.date = bDetailCont
          .bookingDetail.value.dayCareDate.dateInyyyyMMddFormat
          .formatDateYYYYmmdd();
      bookDayCareReq.dropOfftime =
          "${bDetailCont.bookingDetail.value.dayCareDate.dateInyyyyMMddFormat.formatDateYYYYmmdd()} ${bDetailCont.bookingDetail.value.dropoffTime}"
              .dateInyyyyMMddHHmmFormat
              .formatTimeHHmm24hour()
              .toString();
      bookDayCareReq.pickUptime =
          "${bDetailCont.bookingDetail.value.dayCareDate.dateInyyyyMMddFormat.formatDateYYYYmmdd()} ${bDetailCont.bookingDetail.value.pickupTime}"
              .dateInyyyyMMddHHmmFormat
              .formatTimeHHmm24hour()
              .toString();
  ...
  void getPetDaycareAmount() {
    try {
      ...
      if (homeController.dashboardData.value.petDaycareAmount.isNotEmpty) {
        petDaycareAmount(
            homeController.dashboardData.value.petDaycareAmount.first);
        currentSelectedService.value.serviceAmount =
            petDaycareAmount.value.val.toDouble();
  ...
  getDayCareTakers({String searchtext = ""}) {
    ...
    PetServiceFormApis.getEmployee(
            role: EmployeeKeyConst.dayCare, search: searchtext)
        .then((value) {
      isLoading(false);
      dayCareTakerList(value.data);
      hasErrorFetchingDayCareTaker(false);
      if (isUpdateForm.value) {
        selectedDayCareTaker(dayCareTakerList.firstWhere(
        ...
        currentSelectedService.value.serviceAmount =
            selectedDayCareTaker.value.oneDayAmount;
        bookDayCareReq.price = selectedDayCareTaker.value.oneDayAmount;
        isRefresh(true);
        isRefresh(false);
        isRefresh(true);
        isUpdateForm(false);
  ...
  String get getDropOFFDateTime =>
      "${bookDayCareReq.date.trim()} ${bookDayCareReq.dropOfftime.trim()}";
