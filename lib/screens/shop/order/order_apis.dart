...
class OrderApis {
  static Future<BaseResponseModel> placeOrderAPI(Map request) async {
    return BaseResponseModel.fromJson(await handleResponse(
        await buildHttpResponse(APIEndPoints.placeOrder,
            request: request, method: HttpMethodType.POST)));
  }

  static Future<OrderStatusModel> getOrderFilterStatus() async {
    return OrderStatusModel.fromJson(await handleResponse(
  ...
  static Future<dynamic> updateOrderReview({
    ...
    if (employeeId.isNotEmpty)
      multiPartRequest.fields[ProductModelKey.employeeId] = employeeId;
  ...
  static Future<BaseResponseModel> updateDeliveryStatus(
      {required Map request}) async {
    return BaseResponseModel.fromJson(await handleResponse(
        await buildHttpResponse(APIEndPoints.updateDeliveryStatus,
            request: request, method: HttpMethodType.POST)));
  }

  static Future<OrderDetailModel> getOrderDetail(
    ...
    return OrderDetailModel.fromJson(await handleResponse(await buildHttpResponse(
        '${APIEndPoints.getOrderDetails}?order_id=$orderId&order_item_id=$orderItemId$notificationId',
        method: HttpMethodType.GET)));
  }
  ...
  static Future<List<CartListData>> getOrderList({
    ...
    String statusFilter =
        filterByStatus.isNotEmpty ? '&delivery_status=$filterByStatus' : '';
    final orderRes = OrderListModel.fromJson(await handleResponse(
        await buildHttpResponse(
            "${APIEndPoints.getOrderList}?page=$page&per_page=$perPage$statusFilter",
            method: HttpMethodType.GET)));
